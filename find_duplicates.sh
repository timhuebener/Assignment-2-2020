...
do 
    ...
    charCount=0
    # TODO: check this because it may be wrong (probably only the files from the subfolders of src folder will be counted)
    # (maybe the .js files outside this folder or in other folders are also needed)
    # exclude intro, outro, min, build and test files and folders and leave everything else instead
...
do
    ...
    do
        ...
        let divider=${repoCharCounts[${versions[${i}]}]}+${repoCharCounts[$j]}
        echo "DIVIDER ${divider}"
        similarity=$( bc <<< "scale=4; $duplicate_char_count/$divider" )
        echo "---- SIMILARITY between ${versions[${i}]} and ${j}: ${similarity}"
